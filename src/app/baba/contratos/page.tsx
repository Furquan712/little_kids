...
export default async function NannyContractsPage() {
  const auth = await requireRole("NANNY");
  if (!auth.ok) return null;

  const contracts = await listContractsForUser(auth.user.id, "NANNY");
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        {contracts.map((c) => (
          <Link key={c.placementId} href={`/baba/contratos/${c.contractId}`}>
